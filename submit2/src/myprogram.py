...
class MyModel:
    ...
    def generate_all(cls, current_string, next_char):
        ...
        if current_string in cls.bigrams_map: 
            ...
            for tup_word in words:
                ...
                if len(tup_word[0]) > len(next_char) and tup_word[0][len(next_char)] not in seen_characters:
    ...
    def generate_word(cls, s2):
        ...
        for tup_word in words:
            ...
            if len(top_tup_words) == 3:
                break
            if len(cur_char) > len(s2) and cur_char[len(s2)] not in seen_characters:
        ...
        if len(suggestions) < 3:
            ...
            for i in range(3):
                if len(new_suggestions) - 1 < i:
                    suggestions.append((s2.join("s"), 0))
